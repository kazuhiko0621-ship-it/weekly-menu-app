...
## 7. Apple Watch から音声で買い物メモを追加する

買い物リスト画面の「手動で追加」セクションには、Apple Watch のショートカットから音声で品名を追加できます。

### 7-1. Edge Function のデプロイと合言葉の設定

ショートカットは Supabase にログインできないため、合言葉(自分で決めた長い文字列)で認証します。

```powershell
supabase functions deploy quick-add --no-verify-jwt
supabase secrets set QUICK_ADD_SECRET=<自分で決めた長いランダム文字列>
```

- `--no-verify-jwt` は、ショートカットが JWT を持たないため必須です(代わりに合言葉で保護します)
- 合言葉は推測されにくい長い文字列にしてください(例: パスワード生成ツールで作った40文字程度)
- 材料マスタとの照合に `NOTION_TOKEN` / `NOTION_INGREDIENT_DB_ID` を使いますが、これらは既存のものをそのまま流用するので追加設定は不要です

### 7-2. iPhone でショートカットを作る

iPhone の「ショートカット」アプリで新規ショートカットを作成し、以下の3つのアクションを順に追加します。

1. **「テキストを音声入力」**(Dictate Text)
   - 言語: 日本語
2. **「URLの内容を取得」**(Get Contents of URL)
   - URL: `https://<あなたのプロジェクトref>.supabase.co/functions/v1/quick-add`
   - 方法: **POST**
   - ヘッダ:
     - `Content-Type` : `application/json`
     - `x-quick-add-secret` : 7-1 で設定した合言葉
   - 本文: **JSON** を選び、キー `text` の値に、手順1の「音声入力されたテキスト」変数を指定
3. **「通知を表示」**(Show Notification) ※任意
   - 手順2の結果の `message` を表示すると、追加された品名を確認できます

ショートカット名は「買い物リストに追加」など、Siri で呼びやすい名前にしてください。

### 7-3. Apple Watch から使う

ショートカットは iPhone と自動で同期されるので、Apple Watch の「ショートカット」アプリを開けば一覧に表示されます。

- Watch のショートカットアプリからタップして実行
- または「Hey Siri、買い物リストに追加」と話しかける
- 文字盤にコンプリケーションとして配置しておくとワンタップで起動できます

読点区切りで複数まとめて登録できます(例: 「牛乳、卵、トイレットペーパー」)。

> **補足**: watchOS ではショートカットの一部の処理が iPhone 側に中継されて実行されることがあります。iPhone が手元にある、または Watch がセルラー/Wi-Fi に接続されている状態が確実です。

## 既知の制限・改善ポイント

- 「手動で追加」した品(音声入力/手入力)は `shopping_extra_items` テーブルに保存され、**献立からの買い物リストを作り直しても消えません**。削除はゴミ箱アイコン、またはチェック済みをまとめて削除する操作でのみ行えます
- 手動追加した品名が材料マスタ(DB_材料リスト)に一致した場合はそのカテゴリでグループ化され、一致しないものは「その他」にまとまります
- アプリ画面内の音声入力はブラウザの音声認識機能(Web Speech API)を使います。iOS Safari では利用できますが、対応していないブラウザではマイクボタンを押すとメッセージが表示されます
- 買い物リスト機能の追加に伴い、`shopping_extra_items` テーブルを追加しています。**`supabase/schema.sql` の再実行**で反映されます
- 買い物リストは、Notionの **「DB_レシピ材料明細」** と **「DB_材料リスト」** を参照して集計します。反映には以下が必要です
  ```powershell
  supabase functions deploy shopping-list
  supabase secrets set NOTION_DETAIL_DB_ID=3522414344d14a18b9e7a90e6054e4de NOTION_INGREDIENT_DB_ID=22f1ee7b33f181a8bd61da58deb6f05a
  ```
  ```
  Supabase SQL Editorで supabase/schema.sql を再実行
  ```
  ※ 2つのデータベースにも、Notionインテグレーションの接続(Connections)を許可しておく必要があります
- 集計は「DB_材料リスト」の**標準単位**に統一されるため、レシピごとの単位ゆれは発生しません。材料マスタの**常備調味料**にチェックが入っているものは「計上不要」として既定では非表示になり、画面上部のトグルで表示を切り替えられます
- 対象は「Notionのレシピから登録した献立」のみです。外食・各自・手入力の献立、および材料明細が未登録のレシピは集計されず、後者は画面に注意書きとして表示されます
